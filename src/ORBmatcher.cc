...
//     std::cout<<"After calcOpticalFlowPyrLK"<<std::endl;
    ...
    //绘制光流匹配图片
//     cv::Mat img_lk=cv::Mat::zeros(LastFrame.image.rows,2*LastFrame.image.cols,CV_8UC1);
//     cv::Mat lastImg=img_lk(cv::Rect(0,0,LastFrame.image.cols,LastFrame.image.rows));
//     cv::Mat curImg=img_lk(cv::Rect(LastFrame.image.cols,0,LastFrame.image.cols,LastFrame.image.rows));
//     cv::Mat image1=LastFrame.image;
//     cv::Mat image2=CurrentFrame.image;
//     image1.copyTo(lastImg);
//     image2.copyTo(curImg);
// 	  
// 	  
//     for(int i=0;i<LastF_uv.size();i++)
//     {
//       cv::circle(img_lk,LastF_uv[i],2,cv::Scalar(0,255,0),-1);
//       cv::Point2f p_tmp;
//       p_tmp.x=SupposeCurrentF_uv[i].x+LastFrame.image.cols;
//       p_tmp.y=SupposeCurrentF_uv[i].y;
//       cv::circle(img_lk,p_tmp,2,cv::Scalar(0,255,0),-1);
//       //cv::line(img_lk,LastF_uv[i],p_tmp,cv::Scalar(0,255,0),0.5);
//     }
//     
//     char lk[256];
//     std::sprintf(lk,"tmp/lk%d.jpg",n);
//     cv::imwrite(lk,img_lk);
...
// 	 Lk_time1_2=Lk_time1_2+ std::chrono::duration_cast<std::chrono::duration<double> >(t4 - t3).count();
		    ...
		    //计算前后帧光流
		    double lk_du=CurrentFrame.mvKeys[bestIdx2].pt.x-LastFrame.mvKeys[i].pt.x;
		    double lk_dv=CurrentFrame.mvKeys[bestIdx2].pt.y-LastFrame.mvKeys[i].pt.y;
		    double lk_distance=sqrt(lk_du*lk_du+lk_dv*lk_dv);
		    CurrentFrame.Lk_distances[bestIdx2]=lk_distance;
